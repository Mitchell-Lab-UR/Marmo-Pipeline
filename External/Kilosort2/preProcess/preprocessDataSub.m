...
for ibatch = 1:Nbatch
    ...
    % LNR, 60hz line noise removal
%     if getOr(ops, 'LNR', 1)
%         dataraw=double(gather(dataRaw));
%         for i=1:size(dataraw,2)
%             dataraw(:,i) = preprocess.removeLineNoiseChunkwise(double(dataraw(:,i)),ops.fs,[60 120 180]);
%         end
...
%     end
    ...
    % --- remove artifacts (introduced by jake)
    if isfield(ops, 'artifactThresh')
