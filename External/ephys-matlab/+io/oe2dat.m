...
for i = 1:numShanks
    shankPaths{i} = sprintf('_processed%02.0f_%s', i, shanks{i}.name);
end
...
% if no headstage was supplied, assume blank channel map
if strcmp(shanks{1}.name,'_')
    % build channel map
    
    numChan = numel(dir(fullfile(oepath, sprintf('*CH*.continuous') )));
    
    chanMap = (1:numChan)';
    xcoords = ones(numChan,1);
    ycoords = (numChan:-1:1)'*35;
    zcoords = ones(numChan,1);
...
for j = 1:numChannels
    ...
    % if (special flag) 
    %  if ch == X
    %     fileload = sprintf('*AUX1.continuous');
    %  end
    %  if ch == Y
    % 
    %  end
...
%     if (j <= 6)
%         disp('Using AUX channel in place of CH');
%         tmp = dir(fullfile(oepath, sprintf('*AUX%d.continuous',j) ));
%         % if separate files are saved by open ephys gui
%         tmp_ = dir(fullfile(oepath, sprintf('*AUX%d_*.continuous', j) ));
%     else
...
if any(nblocks==0)
	warning('some channels are missing. this is concerning')
    nBlocks(nBlocks==0) = [];
end
...
% fid = cell(numChannels, 1); % old import method
...
for k = 1:nBlocks
    ...
    NT = numel(data_seg);
    samples  = zeros(numChannels, NT, 'int16');
        
    for j = 1:numChannels
        fprintf('Channel %d\n', j)
        
        if nblocks(j)==0
            warning('Channel is empty. Skipping')
            continue
        end
        filename = fullfile(fs{j}(k).folder, fs{j}(k).name);
...
%         info.bitVolts
%         info_.header.bitVolts   
%         size(data_seg)
%         xx = 1:150000;
%         figure(1);
%         plot((xx/30000),data_seg(xx)+(5*j))
%         input('check');
...
%                                  plot(cumsum([[0 ns_(1:end-1)]; ns_], 2)/SAMPLES_PER_RECORD, [ts_; ts_])
            ...
            dn_ =datenum(info_.header.date_created, 'dd-mmm-yyyy');
        ...
        if numel(data_seg) == NT
            samples(j,:) = data_seg;
        else
            warning('oe2dat: number of samples doesn''t match. putting them in where they belong, but something might be up')
            ss = io.convertTimeToSamples(timestamps_seg, info.sampleRate, info.timestamps, info.fragments);
            samples(j,ss) = data_seg;
        end
...
connected(nblocks==0) = false; % dead channels
fs        = ops.fs;
