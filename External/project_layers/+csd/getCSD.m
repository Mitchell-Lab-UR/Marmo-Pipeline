...
%
%  JM revised 6/28/2024 to auto identify dead channels from STA and remove
%      (the dead channels come out flat with very weak modulation compared
%       to the adjacent channels next to them (removed if < 25% range of
...
%         stats.spectrogram(shankInd) = pss;
    ...
    %******** find dead channels and interpolate them
    newsta = sta;
    for k = 1:size(sta,2)
        %**** find neighbor channels
        if (k > 1)
            achan = sta(:,k-1);
            arange = range(achan);
        else
            achan = nan(size(sta(:,k)));
            arange = NaN;
        end
        if (k < size(sta,2))
            bchan = sta(:,k+1);
            ...
            bchan = nan(size(sta(:,k)));
            brange = NaN;
        end
        crange = range(sta(:,k));  % range of current channel
        if (crange < 0.25*min(arange,brange) )  % if your range is less than 10% of the smaller of adjacent channels
           newsta(:,k) = nanmean( [achan' ; bchan'] )';
           disp(sprintf('Found dead channel %d, removing by interpolation (%6.2f,%6.2f) range vs %6.2f',k,arange,brange,crange));
        end
    end
    
    if (0)  % figure for debugging
        size(sta)
        size(newsta)
        figure(1);
        subplot('Position',[0.1 0.1 0.3 0.8]);
        for k = 1:size(sta,2)
           plot(1:size(sta,1),(k*100)+sta(:,k)','k-'); hold on;
           plot(1:size(sta,1),((k*100)*ones(1,size(sta,1))),'k:'); hold on;
        end
        xlabel('Time (ms)');
        ylabel('Voltage on channel');
        title('Original channels');
        subplot('Position',[0.6 0.1 0.3 0.8]);
        for k = 1:size(newsta,2)
           plot(1:size(newsta,1),(k*100)+newsta(:,k)','k-'); hold on;
           plot(1:size(newsta,1),((k*100)*ones(1,size(newsta,1))),'k:'); hold on;
        end
        xlabel('Time (ms)');
        ylabel('Voltage on channel');
        title('After interpolating dead-channels');
        input('check');
        close(1);
    end
    %**** replace sta with the new interpolated version after dead-channel
    %**** removal
    sta = newsta;
    %***************     
...
%     imagesc(time, ch0, pss)
    ...
    if 0 %exclude % JFM and NM removed, needs to be fixed another way in future
