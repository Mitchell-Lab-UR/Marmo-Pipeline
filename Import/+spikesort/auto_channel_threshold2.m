function [sp,fthresh] = auto_channel_threshold2(spdata,spinfo,FullSpread, UseThresh)
...
%****           fthresh - negative voltaged used for thresholding spikes
...
for iCh = 1:nChannels
  ...
  while (got_thresh == 0)
    ...
    if ~(UseThresh) % if not use thresh
     if (FullSpread)
      ...
      title(sprintf('Mean Rate %5.2f Hz Chan(%d)',mrate,iCh));
      fprintf('Current threshold: %5.2f std\n',threshold);
      nval = [];  % accept the threshold given (don't change it)   
    end
    %***************
    if isempty(nval)
       got_thresh = 1;
