function [hash,smallfp] = compute_hash_channels_interval2(DataFolder,ShankMap,Interval)
%*******
%** function hash = compute_hash_channels_interval2(DataFolder,ShankMap,Interval)
%***
%*** input:
...
%**************
  ...
  %***** first pass, due auto channel thresholding and get threshold per
  thresholds = [];
  for k = 1:ChanNum
       ...
       [~,fthresh] = spikesort.auto_channel_threshold2(spdata,spinfo, hash.BaseRate, 0);
       %******************
       if ~isnan(fthresh) && (fthresh < 0)
          thresholds = [thresholds ; fthresh];
       end
  end
  mthreshold = median(thresholds);   % use this common threshold all channels
  %**************
  disp('Ready to see thresholds');
  thresholds
  disp(sprintf('Mean threshold %8.6f',mthreshold));
  disp('Applying uniform threshold to channels');
  ...
  for k = 1:ChanNum
       ...
       %********* now filter the raw data **************
       disp(sprintf('Hash spike sorting continuous file %s for %d',tag,kmap));
       hash.sp{kmap} = spikesort.auto_channel_threshold2(spdata,spinfo, hash.BaseRate, mthreshold);
