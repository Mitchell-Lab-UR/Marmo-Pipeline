...
% DroppedBit = 1;
% if (DroppedBit == 1)
%   %******** SPECIAL FOR Feb 26, 2020 ************
%   % TAGSTART = TAGSTART - 32;
%   % TAGEND = TAGEND - 32;
%   k = 1;
%   while (k <= length(strobes))
%       strobes(k) = strobes(k) + 32;
%       k = k + 7;
%   end
% end
...
%for k = 427:450 % 127:size(Exp.D,1)
   StartTag = Exp.D{k}.STARTCLOCK;
   EndTag = Exp.D{k}.ENDCLOCK;
   if (DroppedBit == 1)
      if (StartTag(5) >= 32) 
          StartTag(5) = StartTag(5)-32;
      end
      if (StartTag(6) >= 32) 
          StartTag(6) = StartTag(6)-32;
      end
      if (EndTag(5) >= 32) 
          EndTag(5) = EndTag(5)-32;
      end
      if (EndTag(6) >= 32) 
          EndTag(6) = EndTag(6)-32;
      end
      start = synchtime.find_strobe_time_droppedbit(TAGSTART,StartTag,strobes,tstrobes);
      finish = synchtime.find_strobe_time_droppedbit(TAGEND,EndTag,strobes,tstrobes);
   else
     start = synchtime.find_strobe_time(TAGSTART,StartTag,strobes,tstrobes);
     finish = synchtime.find_strobe_time(TAGEND,EndTag,strobes,tstrobes);
   end
